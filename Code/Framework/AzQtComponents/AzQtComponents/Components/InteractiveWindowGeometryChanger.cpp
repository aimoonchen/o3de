...
namespace AzQtComponents
{
    ...
    void InteractiveWindowResizer::handleMouseMove(QMouseEvent* ev)
    {
        ...
        if (m_sideToResize & LeftSide)
        {
            geometry.setLeft(ev->globalPosition().toPoint().x() - 1);
        }
        if (m_sideToResize & RightSide)
        {
            geometry.setRight(ev->globalPosition().toPoint().x());
        }
        if (m_sideToResize & TopSide)
        {
            geometry.setTop(ev->globalPosition().toPoint().y() - 1);
        }
        if (m_sideToResize & BottomSide)
        {
            geometry.setBottom(ev->globalPosition().toPoint().y());
        }
