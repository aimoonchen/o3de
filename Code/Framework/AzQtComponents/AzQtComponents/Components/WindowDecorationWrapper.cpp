...
namespace AzQtComponents
{
    ...
    {
        QRect availableGeometry = window->screen()->availableGeometry();
    ...
    bool WindowDecorationWrapper::handleNativeEvent(const QByteArray& eventType, void* message, qintptr* result, const QWidget* widget)
    {
        ...
        if (msg->message == WM_GETMINMAXINFO && widget->isMaximized())
        {
            ...
            // WM_GETMINMAXINFO is only handled for the primary screen (guarded above), whose
            // native origin is (0,0), so native = logical * devicePixelRatio - the public-API
            // equivalent of QHighDpi::toNativePixels (private in Qt 6).
            const qreal dpr = screen->devicePixelRatio();
            const QRect availableGeometry(
                QPoint(qRound(screen->availableGeometry().x() * dpr), qRound(screen->availableGeometry().y() * dpr)),
                screen->availableGeometry().size() * dpr);
