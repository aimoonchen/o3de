...
namespace AzToolsFramework
{
   ...
   bool EntityPropertyEditor::AreComponentsRemovable(AZStd::span<AZ::Component* const> components) const
    ...
    {
        // Qt6: QContextMenuEvent keeps globalPos() (QPoint); it is not a QMouseEvent
        // and has no globalPosition().
    ...
    bool EntityPropertyEditor::HandleMenuEvent(QObject* /*object*/, QEvent* event)
    {
        ...
        case QEvent::MouseMove:
            ...
            QAction* action = menu->actionAt(originalMouseEvent->position().toPoint());
            if (!action)
            {
                m_reorderDropTarget = nullptr;
                break;
        ...
        QWidget* widget, const QVector<QAction*>& actions, const bool enable)
    {
        ...
        // Qt6: QWidget::addAction is overloaded (convenience overloads returning QAction*),
        // so the member-pointer must be disambiguated with an explicit cast to the
        // void(QAction*) signature.
        const AddRemoveFunc addRemove = enable
            ? static_cast<AddRemoveFunc>(&QWidget::addAction)
            : static_cast<AddRemoveFunc>(&QWidget::removeAction);
