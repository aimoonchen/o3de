    ...
    bool LUAEditorPlainTextEdit::HandleHomeKeyPress(QKeyEvent* event)
    {
        if (event->key() == Qt::Key::Key_Home)
        {
            ...
            if (block.isValid())
            {
                ...
                {
                    QString text = block.text();
                    QStringView line = QStringView(text).mid(0, cursor.columnNumber());
                ...
                else
                {
                    ...
                    QStringView line = QStringView(text).mid(0, position);
        ...
        if (event->key() == Qt::Key::Key_Tab || event->key() == Qt::Key::Key_Backtab)
        {
            ...
            else
            {
                ...
                else if (event->key() == Qt::Key::Key_Backtab)
                {
                    ...
                    if (text[columnNumber] != '\t' || QStringView(text).mid(columnNumber, m_tabSize) != tabString)
                    {
                        ...
                        else if (columnNumber > 0 && QStringView(text).mid(columnNumber - m_tabSize, m_tabSize) == tabString)
                        {
