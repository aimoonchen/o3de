...
def init_ly_pyside(env_dict=_SYNTH_ENV_DICT):
    ...
    if sys.platform.startswith('win'):
        ...
        _LOGGER.debug('PySide6 bootstrapped PATH for Windows.')

    try:
        import PySide6
        _LOGGER.debug('DCCsi, config.py: SUCCESS: import PySide6')
        _LOGGER.debug(PySide6)
        status = True
    except ImportError as e:
        _LOGGER.debug('DCCsi, config.py: FAILURE: import PySide6')
        status = False
        raise(e)

    try:
        import shiboken6
        _LOGGER.debug('DCCsi, config.py: SUCCESS: import shiboken6')
        _LOGGER.debug(shiboken6)
        status = True
    except ImportError as e:
        _LOGGER.debug('DCCsi, config.py: FAILURE: import shiboken6')
        status = False
        raise(e)
...
def test_Qt():
    try:
        import PySide6
        _LOGGER.info('PySide2: {0}'.format(Path(PySide6.__file__).as_posix()))
