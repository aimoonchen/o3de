...
// Vendor copy of AtomToolsFramework DynamicProperty for CrossEngineEditor.
// Original: Gems/Atom/Tools/AtomToolsFramework/Code/Source/DynamicProperty/
// Upstream commit: afcd2d2422
// Modifications: removed Atom RPI dependency (ColorUtils), inlined color config.

#include <MaterialEditor/Vendor/AtomToolsFramework/DynamicProperty/DynamicProperty.h>
#include <AzCore/Serialization/SerializeContext.h>
#include <AzCore/std/algorithm.h>

// S2: For ColorEditorConfiguration - defines the color space picker behavior.
// This header is in AzToolsFramework (not Atom RPI), so it's available without Atom dependencies.
...
namespace AtomToolsFramework
{
    ...
    void DynamicProperty::UpdateEditData()
    {
        if (m_editDataTracker != &m_editData)
        {
            ...
            {
                // S2: Use the configured color space instead of always Linear.
                // m_colorSpace 0 = original behavior, 1 = sRGB editor config.
                // ColorUtils lives in Atom RPI.Edit (not in Core target).
                // For Core-only builds, use a fallback config with the same behavior.
                auto colorConfig = AzToolsFramework::ColorEditorConfiguration{};
                colorConfig.m_colorPickerDialogConfiguration = AzQtComponents::ColorPicker::Configuration::RGB;
                colorConfig.m_colorSpaceNames[0] = "Linear sRGB";
                colorConfig.m_colorSpaceNames[1] = "sRGB";
                colorConfig.m_propertyColorSpaceId = m_config.m_colorSpace;
                colorConfig.m_colorPickerDialogColorSpaceId = 1;
                colorConfig.m_colorSwatchColorSpaceId = 1;
                colorConfig.m_transformColorCallback = [](const AZ::Color& c, AZ::u32 fromCs, AZ::u32 toCs) -> AZ::Color
                {
                    if (fromCs == toCs) return c;
                    if (fromCs == 0 && toCs == 1) return c.LinearToGamma();
                    if (fromCs == 1 && toCs == 0) return c.GammaToLinear();
                    return c;
                };
                AddEditDataAttribute(AZ_CRC_CE("ColorEditorConfiguration"), colorConfig);
            }

            // S5: Forward file extension hints to the asset picker when non-empty.
            if (!m_config.m_fileExtensions.empty())
            {
                AddEditDataAttribute(AZ_CRC_CE("Extensions"), m_config.m_fileExtensions);
            }

            // S6: Forward suffix string (e.g. unit label) when non-empty.
            if (!m_config.m_suffix.empty())
            {
                AddEditDataAttribute(AZ::Edit::Attributes::Suffix, m_config.m_suffix);
            }
