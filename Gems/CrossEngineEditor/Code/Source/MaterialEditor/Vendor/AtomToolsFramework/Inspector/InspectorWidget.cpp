...
// Vendor copy of AtomToolsFramework InspectorWidget for CrossEngineEditor.
// Original: Gems/Atom/Tools/AtomToolsFramework/Code/Source/Inspector/InspectorWidget.cpp
// Upstream commit: ba9a6beab4
...
#include <MaterialEditor/Vendor/AtomToolsFramework/Util/Util.h>
...
namespace AtomToolsFramework
{
    ...
    InspectorWidget::InspectorWidget(QWidget* parent)
        : QWidget(parent)
    {
        // CEE: inline layout instead of AUTOUIC-generated Ui class.
        auto* mainLayout = new QVBoxLayout(this);
        mainLayout->setContentsMargins(0, 0, 0, 0);
        mainLayout->setSpacing(0);

        m_headingContents = new QWidget(this);
        m_headingContentsLayout = new QVBoxLayout(m_headingContents);
        m_headingContentsLayout->setContentsMargins(0, 0, 0, 0);
        mainLayout->addWidget(m_headingContents);

        m_groupScrollArea = new QScrollArea(this);
        m_groupScrollArea->setWidgetResizable(true);
        m_groupScrollArea->setFrameShape(QFrame::NoFrame);
        mainLayout->addWidget(m_groupScrollArea);

        m_groupContents = new QWidget();
        m_groupContentsLayout = new QVBoxLayout(m_groupContents);
        m_groupContentsLayout->setContentsMargins(0, 0, 0, 0);
        m_groupScrollArea->setWidget(m_groupContents);
    ...
    void InspectorWidget::AddHeading(QWidget* headingWidget)
    {
        ...
        m_headingContentsLayout->addWidget(headingWidget);
    }

    void InspectorWidget::ClearHeading()
    {
        while (QLayoutItem* child = m_headingContentsLayout->takeAt(0))
        {
            delete child->widget();
            delete child;
        }
    ...
    {
        while (QLayoutItem* child = m_groupContentsLayout->takeAt(0))
        {
            delete child->widget();
            delete child;
        }
    ...
    {
        m_groupContentsLayout->addStretch();

        // Scroll to top whenever there is new content
        m_groupScrollArea->verticalScrollBar()->setValue(m_groupScrollArea->verticalScrollBar()->minimum());
    ...
    void InspectorWidget::AddGroup(
        ...
        QWidget* groupWidget)
    {
        InspectorGroupHeaderWidget* groupHeader = CreateGroupHeader(m_groupContents);
        groupHeader->setText(groupDisplayName.c_str());
        groupHeader->setToolTip(groupDescription.c_str());
        m_groupContentsLayout->addWidget(groupHeader);

        groupWidget->setObjectName(groupName.c_str());
        groupWidget->setParent(m_groupContents);
        m_groupContentsLayout->addWidget(groupWidget);
    ...
    {
        for (auto groupWidget : m_groupContents->findChildren<InspectorGroupWidget*>(groupName.c_str()))
        {
            groupWidget->Refresh();
        }
    }
    ...
    {
        for (auto groupWidget : m_groupContents->findChildren<InspectorGroupWidget*>())
        {
            groupWidget->Refresh();
        }
    }
    ...
    InspectorGroupHeaderWidget* InspectorWidget::CreateGroupHeader(QWidget* parent)
    {
        return new InspectorGroupHeaderWidget(parent);
    }
} // namespace AtomToolsFramework

#include <MaterialEditor/Vendor/AtomToolsFramework/Inspector/moc_InspectorWidget.cpp>
