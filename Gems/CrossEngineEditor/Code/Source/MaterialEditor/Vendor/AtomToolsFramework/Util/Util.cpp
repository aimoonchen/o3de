...
// Vendor copy of AtomToolsFramework Util for CrossEngineEditor.
// Original: Gems/Atom/Tools/AtomToolsFramework/Code/Source/Util/Util.cpp
// Upstream commit: ba9a6beab4
// Modifications: removed Atom-specific includes, added Atom-free path resolution.
...
{
    AZStd::string ResolvePathReference(const AZStd::string& scanFolder, const AZStd::string& relativePath)
    {
        // S1: Atom-free path resolution. Simply concatenates and normalizes.
        // If the relative path is already absolute, return it as-is (same as the Atom version).
        ...
        AZ::IO::FixedMaxPath result(scanFolder);
        result /= AZ::IO::PathView(relativePath);
        return result.LexicallyNormal().StringAsPosix();
    }
    ...
    AZStd::string GetUniqueFilePath(const AZStd::string& initialPath)
    {
        ...
        {
            fileInfo.setFile(QString("%1_%2%3").arg(basePathAndName).arg(++counter).arg(extension));
        }
    ...
    bool ValidateDocumentPath(AZStd::string& path)
    {
        ...
        // CEE: use AZ::StringFunc instead of AzFramework::StringFunc::Path.
        AZ::StringFunc::Path::Normalize(path);

        if (AZ::StringFunc::Path::IsRelative(path.c_str()))
        {
            return false;
        }
    ...
    bool IsDocumentPathEditable([[maybe_unused]] const AZStd::string& path)
    {
        return true;
    }

    bool IsDocumentPathPreviewable([[maybe_unused]] const AZStd::string& path)
    {
        return true;
    }
    ...
    AZStd::set<AZStd::string> GetPathsFromMimeData(const QMimeData* mimeData)
    {
        ...
        if (mimeData->hasFormat(AzToolsFramework::EditorAssetMimeDataContainer::GetMimeType()))
        {
            ...
            if (container.FromMimeData(mimeData))
            {
                ...
                {
                    // CEE: simplified - skip asset resolution for materials not in supported folders.
                    (void)asset;
                }
            }
        }
        ...
        const AZStd::string& folder, const AZStd::function<bool(const AZStd::string&)> visitorFn, bool recurse)
    {
        ...
        AZ::IO::SystemFile::FindFiles(
            ...
            [&](const char* item, bool is_file)
            {
                ...
                // Continue if we can
                fullPath.clear();
                AZ::StringFunc::Path::Join(folder.c_str(), item, fullPath);
    ...
    bool IsPathIgnored([[maybe_unused]] const AZStd::string& path)
    {
        return false;
    }
    ...
    {
        // CEE: return project root + engine root instead of broadcasting
        // AssetSystemRequestBus (no AssetProcessor in CEE).
        AZStd::vector<AZStd::string> scanFolders;
        AZ::IO::FixedMaxPath projectRoot = AZ::Utils::GetProjectPath();
        if (!projectRoot.empty())
        {
            scanFolders.push_back(projectRoot.c_str());
        }
        AZ::IO::FixedMaxPath engineRoot = AZ::Utils::GetEnginePath();
        if (!engineRoot.empty())
        {
            scanFolders.push_back(engineRoot.c_str());
        }
        return scanFolders;
    }
