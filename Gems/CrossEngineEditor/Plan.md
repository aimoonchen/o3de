...
> **C1 原文不动**——`check_no_atom.ps1` 第 1/2 层由「禁裸词 `Atom`」精化为「禁 Atom **渲染模块**（`Atom/RPI`、`Atom/RHI`、`Atom/Feature`、`AZ::RPI`…）」并白名单 Atom-free 的 `Gem::AtomToolsFramework.Core`，第 3 层（链接产物无 `Atom_*.lib`）不动。理由：C1 规则原文本就只禁「渲染类模块」且明写「不列白名单，以框架实际需要为准」，是**代理指标严于规则本身且严错了对象**（拦名字而非拦依赖）。
