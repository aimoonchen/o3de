...
namespace EMStudio
{
    ...
    void MainWindow::Init()
    {
        ...
        m_mergeActorAction = menu->addAction(tr("&Merge Actor"), QKeySequence(0x0 | Qt::CTRL | Qt::Key_I), this, &MainWindow::OnFileMergeActor);
    ...
    void MainWindow::dropEvent(QDropEvent* event)
    {
        ...
        if (AzToolsFramework::AssetBrowser::Utils::FromMimeData(mimeData, entries))
        {
            ...
            LoadFiles(fileNames, event->position().toPoint().x(), event->position().toPoint().y());
            event->acceptProposedAction();
