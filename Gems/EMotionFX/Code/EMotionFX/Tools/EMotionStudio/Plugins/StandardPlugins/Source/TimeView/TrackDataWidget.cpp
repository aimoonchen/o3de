...
namespace EMStudio
{
    ...
    void TrackDataWidget::mouseDoubleClickEvent(QMouseEvent* event)
    {
        ...
        RecorderGroup* recorderGroup = m_plugin->GetTimeViewToolBar()->GetRecorderGroup();
        if (GetIsInsideNodeHistory(aznumeric_cast<int>(event->position().y())) && recorderGroup->GetDisplayNodeActivity())
        {
            EMotionFX::Recorder::ActorInstanceData* actorInstanceData = FindActorInstanceData();
            EMotionFX::Recorder::NodeHistoryItem* historyItem = FindNodeHistoryItem(actorInstanceData, aznumeric_cast<int>(event->position().x()), aznumeric_cast<int>(event->position().y()));
            if (historyItem)
            {
    ...
    void TrackDataWidget::mouseMoveEvent(QMouseEvent* event)
    {
        ...
        const int32 deltaRelX = aznumeric_cast<int>(event->position().x()) - m_lastMouseX;
        m_lastMouseX = aznumeric_cast<int>(event->position().x());
        m_plugin->m_curMouseX = aznumeric_cast<int>(event->position().x());
        m_plugin->m_curMouseY = aznumeric_cast<int>(event->position().y());

        const int32 deltaRelY = aznumeric_cast<int>(event->position().y()) - m_lastMouseY;
        m_lastMouseY = aznumeric_cast<int>(event->position().y());
        ...
        // get the track over which the cursor is positioned
        TimeTrack* mouseCursorTrack = m_plugin->GetTrackAt(aznumeric_cast<int>(event->position().y()));
        ...
        if (m_mouseLeftClicked)
        {
            ...
            if (m_draggingElement == nullptr && m_resizeElement == nullptr && m_rectSelecting == false)
            {
                ...
                int newX = aznumeric_cast<int>(event->position().x());
            ...
            const int32 deltaX = aznumeric_cast<int>(event->position().x()) - m_lastLeftClickedX;
            ...
            const int32 deltaMovement = aznumeric_cast<int>(event->position().x()) - m_lastMouseMoveX;
            m_lastMouseMoveX = aznumeric_cast<int>(event->position().x());
        ...
        {
            UpdateMouseOverCursor(aznumeric_cast<int>(event->position().x()), aznumeric_cast<int>(event->position().y()));
        }
    }
        ...
        if (event->button() == Qt::LeftButton)
        {
            ...
            else // not inside timeline
            {
                ...
                }
                {
                    ...
                    TimeTrackElement* element = m_plugin->GetElementAt(aznumeric_cast<int>(event->position().x()), aznumeric_cast<int>(event->position().y()));
                    if (element)
                    {
                        // show the time of the currently dragging element in the time info view
                    ...
                    // store the last clicked position
                    m_mouseLeftClicked   = true;
                    m_lastLeftClickedX   = aznumeric_cast<int>(event->position().x());
                }
            }
        }
