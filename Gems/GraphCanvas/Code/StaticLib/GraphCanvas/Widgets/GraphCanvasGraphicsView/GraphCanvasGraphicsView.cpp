    ...
    void GraphCanvasGraphicsView::mouseMoveEvent(QMouseEvent* event)
    {
        ...
        else if (m_checkForDrag && isInteractive())
        {
            ...
            if ((m_initialClick - event->pos()).manhattanLength() > (width() * 0.005f + height() * 0.005f))
            {
                ...
                */
                QMouseEvent startPressEvent(QEvent::MouseButtonPress, m_initialClick, mapToGlobal(m_initialClick), Qt::LeftButton, Qt::LeftButton, event->modifiers());
                QGraphicsView::mousePressEvent(&startPressEvent);

                QMouseEvent customEvent(event->type(), event->pos(), event->globalPosition().toPoint(), Qt::LeftButton, Qt::LeftButton, event->modifiers());
        ...
        if (m_checkForEdges)
        {
            m_edgePanning = CalculateEdgePanning(event->globalPosition().toPoint());
            ManageTickState();
        }
